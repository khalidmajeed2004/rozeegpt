...
PURCHASED=500          # contracted credits — supplied separately, not present in the raw sheet
...
users=k.groupby(k.email.str.lower()).agg(req=('Request_ID','size'),
...
D=dict(purchased=PURCHASED, used=len(k), success=int(d.Request_ID.nunique()),
       failed=len(k)-int(d.Request_ID.nunique()), kpis=len(d),
       users=int(k.email.str.lower().nunique()),
       util=round(len(k)/PURCHASED*100,1), over=len(k)-PURCHASED,
       exh_date=ex.Request_Date.strftime('%d %b %Y'),
       active_days=int(k.Request_Date.dt.date.nunique()),
       first=k.Request_Date.min().strftime('%d %b %Y'),
       last=k.Request_Date.max().strftime('%d %b %Y'),
       months=M.to_dict('records'),
       top_users=[dict(u=e.split('@')[0], req=int(r.req), kpis=int(r.kpis),
                       span=f"{r['first']:%b %y} – {r['last']:%b %y}") for e,r in users.head(10).iterrows()],
       buckets={str(i):int(v) for i,v in buckets.items()},
       median=int(users.req.median()), ten_plus=int((users.req>=10).sum()),
       top10share=round(users.head(10).req.sum()/len(k)*100,1),
       top20share=round(users.head(20).req.sum()/len(k)*100,1),
       jun=int(M.loc[M.m=='2026-06','req'].iloc[0]),
       jun_new_users=int(M.loc[M.m=='2026-06','new_users'].iloc[0]),
       avg_kpi=round(len(d)/int(d.Request_ID.nunique()),1),
       util_success=round(int(d.Request_ID.nunique())/PURCHASED*100,1))
json.dump(D,open('data.json','w'),indent=1)
print({x:D[x] for x in ['purchased','used','util','over','exh_date','users','kpis','median','ten_plus']})
